...
function _isfree_Q32(K::AnticNumberField)
  ...
  fl, y, Ok, Q32toD16, repsD16 = _is_D16_subfield_free(K, KtoQG, QG)

  QD16 = codomain(Q32toD16)

  B, BtoA = [(B, mB) for (B, mB) in res if degree(B) == 16][1]
  e = BtoA(one(B))
  ...
  Lambda_star = _unit_group_generators_quaternion(Lambda)
   ...
   QoverQQ, QtoQoverQQ = restrict_scalars(Q, FlintQQ)
  ...
  for i in 1:length(Lambda_star_in_QGalmost)
    ...
    push!(Lambda_star_in_QG, QG(fmpq[w[1, j] for j in 1:32]))
  end
  ...
  for u in Lambda_star_in_QG
    ured = sum(FlintZZ(u.coeffs[QD16.group_to_base[g]]) * F2D16(g) for g in group(QD16))
...
#
  ...
  return xxlift, y, repsD16, repsLambdastart, Ok, Q32toD16, KtoQG
end
...
function _is_D16_subfield_free(K, KtoQG, QG::AlgGrp)
  ...
  D16, Q32toD16 = _get_D16(QG)
  DD16, ktoD16 = Hecke.galois_module(k, D16)
  @assert DD16 === D16
  ...
  cl = closure(__units, (x, y) -> (x[1] * y[1], x[2] * y[2]), eq = (x, y) -> x[2] == y[2])
  return true, x, Ok, Q32toD16, [x for (x, y) in cl]
end
